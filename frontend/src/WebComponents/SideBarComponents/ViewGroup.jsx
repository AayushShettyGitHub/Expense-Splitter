...
const SelectedTripView = ({ trip, user, onBack, memberById }) => {
    ...
    const fetchExpensesAndSettlements = async () => {
      try {
        const expRes = await axios.get(
          `https://split-backend-263e.onrender.com/api/event/${trip._id}/expenses`,
        ...
        const setRes = await axios.get(
          `https://split-backend-263e.onrender.com/api/event/settlements/${trip._id}`,
          { withCredentials: true }
        );
  ...
  const addExpense = async () => {
    ...
    try {
      await axios.post(
        `https://split-backend-263e.onrender.com/api/event/${trip._id}/expense`,
        {
  ...
  const settleTripExpenses = async () => {
    ...
    try {
      await axios.get(
        `https://split-backend-263e.onrender.com/api/event/${trip._id}/settlements`,
        { withCredentials: true }
      );
                      ...
                      onClick={async () => {
                        try {
                          const res = await axios.patch(
                            `https://split-backend-263e.onrender.com/api/event/mark-paid/${trip._id}/${s._id}`,
                            {},
                            { withCredentials: true }
                          );
...
const ViewGroup = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const gRes = await axios.get(`https://split-backend-263e.onrender.com/api/groups/${selectedGroup._id}`, { withCredentials: true });
        setGroup(gRes.data);
        setSelectedGroup(gRes.data);
        setGroupMembers(gRes.data.members || []);

        const eRes = await axios.get(`https://split-backend-263e.onrender.com/api/group/${selectedGroup._id}/events`, { withCredentials: true });
        const events = Array.isArray(eRes.data?.events) ? eRes.data.events : [];
        setTrips(events.map(normalizeEvent));

        const aRes = await axios.get(`https://split-backend-263e.onrender.com/api/groups/${selectedGroup._id}/events/active`, { withCredentials: true });
  ...
  const toggleActiveTrip = async (tripId) => {
    try {
      if (activeTrips.includes(tripId)) {
        await axios.delete(
          `https://split-backend-263e.onrender.com/api/groups/${group._id}/events/${tripId}/active`,
          { withCredentials: true }
        );
  ...
  const handleKickMember = async (memberId) => {
    try {
      await axios.post(`https://split-backend-263e.onrender.com/api/kick/${group._id}/${memberId}`, {}, { withCredentials: true });
      const updated = await axios.get(`https://split-backend-263e.onrender.com/api/groups/${group._id}`, { withCredentials: true });
  ...
  const handleInviteMember = async () => {
    ...
    try {
      await axios.post(`https://split-backend-263e.onrender.com/api/send-invite/${group._id}`, { email: inviteEmail }, { withCredentials: true });
      const updated = await axios.get(`https://split-backend-263e.onrender.com/api/groups/${group._id}`, { withCredentials: true });
      setGroup(updated.data);
      setGroupMembers(updated.data.members || []);
      setInviteEmail("");
  ...
  const createTrip = async () => {
    ...
    try {
      const res = await axios.post(
        `https://split-backend-263e.onrender.com/api/group/${group._id}/event`,
